...
def get_metric_xlim(df, metric):
    ...
    if metric == "ssim":
        x_min = values.quantile(0.0)
        x_max = values.quantile(1.0)
...
def plot_metric_hexbin(df, metric, out_path):
    ...
    fig, ax = plt.subplots(figsize=(10, max(5, 2.5 * len(present_order))))
    ...
    hb = ax.hexbin(
        ...
        bins=50,
        gridsize=(100, max(25, 25 * len(present_order))),
...
def plot_all_metrics(df, out_dir):
    ...
    for metric in ["ssim", "psnr", "cosine"]:
        ...
        plot_metric_box(df, metric, out_dir / f"{metric}_reconstruction_box_plot.png")

        plot_metric_hexbin(df, metric, out_dir / f"{metric}_reconstruction_hexbin.png")
